...
def create_video_description(scheduled_match: Match) -> str:
    ...
    match_part_prefix = "the game" if scheduled_match.format == Match.Format.BEST_OF_1 else "all games"
    match_part = f"Highlights from {match_part_prefix} between {scheduled_match.team_1.organization} and " \
                 f"{scheduled_match.team_2.organization} ({scheduled_match.get_format_display()})\n" \
                 f"{scheduled_match.team_1.organization}: TEAM_1_PLAYERS\n" \
    ...
    tags_part = f"#{str(scheduled_match.team_1.organization).replace(' ', '').lower()} " \
                f"#{str(scheduled_match.team_2.organization).replace(' ', '').lower()} " \
...
def create_team_logo_thumbnail_part(team: Team) -> Image.Image:
    ...
    longest_side = 0 if logo.size[0] >= logo.size[1] else 1

    new_longest_side = 250
    scale = (new_longest_side / float(logo.size[longest_side]))
    new_shortest_side = int((float(logo.size[1 - longest_side]) * float(scale)))
    logo = logo.resize((new_shortest_side if longest_side == 1 else new_longest_side,
                        new_longest_side if longest_side == 1 else new_shortest_side), Image.Resampling.LANCZOS)
